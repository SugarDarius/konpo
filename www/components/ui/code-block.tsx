...
const removeAnnotations = (code: string): string => {
  return code
    .split('\n')
...
export async function CodeBlock({
  ...
  return (
    <div
      className={cn(
        'group relative isolate flex min-h-11 w-full flex-col overflow-hidden rounded-lg bg-muted',
        className
      )}
      {...props}
    >
      <CopyButton
        ...
        text={removeAnnotations(code)}
      />
      <div
        className={cn(
          'flex flex-1 overflow-x-auto font-mono text-secondary-foreground text-sm',
